...
bool Picker::visit(SgShapeNode& node) {
    shared_ptr<SgRbtNode> node_rbt = shared_ptr<SgRbtNode>();
  if(dynamic_pointer_cast<SgRbtNode>(nodeStack_.back()))
        node_rbt = dynamic_pointer_cast<SgRbtNode>(nodeStack_.back());
  idCounter_++;
  Cvec3 color = idToColor(idCounter_);
      
  addToMap(idCounter_, node_rbt);
  nodeStack_.push_back(node.shared_from_this());
  return drawer_.visit(node);
}
...
shared_ptr<SgRbtNode> Picker::getRbtNodeAtXY(int x, int y) {
  ...
  int out_id = colorToId(p);
  return find(out_id); // return null for now
}
